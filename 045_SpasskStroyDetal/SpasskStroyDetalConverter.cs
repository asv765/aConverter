...
namespace _045_SpasskStroyDetal
{
            ...
            foreach (var abonent in aList)
            {
                ...
                foreach (var group in grouped)
                {
                    ...
                    if (notNull.Any())
                    {
                        ...
                        {
                            if (!services.Contains(money.ServiceKod))
            ...
            foreach (Abonent abonent in aList)
            {
                foreach (var counter in abonent.CounterList)
                {
                    var c = new CNV_COUNTER
                    {
                        LSHET = Consts.GetLs(abonent.Ls),
                        CNTTYPE = 1,
                        COUNTERID = String.Format("9{0}{1}", abonent.Ls, counter.CounterNum)
                    };
                    lcn.Add(c);
                }
                Iterate();
            ...
            int i = 0;
                foreach (Abonent abonent in aList)
                {
                    foreach (var money in abonent.MoneyList)
                    {
                        ...
                        var documentcd = String.Format("N{0}_{1}", lshet.TrimStart('0'), i);
                        ...
                        var ndef = new CNV_NACH
                        {
                            ...
                            nm.RegisterNach(ndef, lshet, 04, 2016, money.Nath, 0, new DateTime(2016,04,01), documentcd);

                            nm.RegisterEndSaldo(lshet,04,2016,servicecd, servicename, money.Saldo );
                    }
                    Iterate();
                }
                StepFinish();
    ...
    public class ConvertCharsNew : ConvertCase
    {
        ...
        public override void DoConvert()
        {
            ...
            DataTable recodeTable = Utils.ReadExcelFile(@"D:\Work\C#\C#Projects\aConverter\045_SpasskStroyDetal\Sources\Таблица перекодировки1С_chars.xlsx", "Лист1");
            var lcc = new List<CNV_CHAR>();
    ...
    //            }
                ...
                var kosheAbonent =
    aList.SingleOrDefault(al => al.Uklshet == Consts.GetLs(dataRow[0].ToString(), dataRow[1].ToString()));
                ...
                {
                    if (Convert.ToInt32(dataRow["TARIFCD"].ToString()) < Convert.ToInt32(dt.Rows[i - 1]["TARIFCD"].ToString()))
                    {
                        Iterate();
                        i++;
                        continue;
