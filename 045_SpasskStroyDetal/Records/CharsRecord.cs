...
namespace _044_TeplComp
{
    ...
    public partial class CharsRecord : AbstractRecord
    {
        ...
        {
            string rs = String.Format("INSERT INTO CHARS (GRKOD, LSHET, CHARCD, CHARNAME, VALUE, DATE) VALUES ('{0}', '{1}', {2}, '{3}', {4}, CTOD('{5}'))", String.IsNullOrEmpty(Grkod) ? "" : Grkod.Trim(), String.IsNullOrEmpty(Lshet) ? "" : Lshet.Trim(), Charcd.ToString(), String.IsNullOrEmpty(Charname) ? "" : Charname.Trim(), Value_.ToString().Replace(',', '.'), Date == DateTime.MinValue ? "" : String.Format("{0}/{1}/{2}", Date.Month, Date.Day, Date.Year));
            return rs;
        }
    }
}