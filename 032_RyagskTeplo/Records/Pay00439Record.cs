...
{
    [TableName("PAY00439.DBF")]
    public partial class Pay00439Record: AbstractRecord
    {
        private Int64 org;
        // <summary>
        // ORG N(6)
        ...
        {
            Pay00439Record retValue = new Pay00439Record();
        ...
        {
            string rs = String.Format("INSERT INTO PAY00439 (ORG, LS_ORG, DAT, SALDO, SUMN, SUMO) VALUES ({0}, '{1}', '{2}', {3}, {4}, {5})", Org.ToString(), String.IsNullOrEmpty(Ls_org) ? "" : Ls_org.Trim(), String.IsNullOrEmpty(Dat) ? "" : Dat.Trim(), Saldo.ToString().Replace(',','.'), Sumn.ToString().Replace(',','.'), Sumo.ToString().Replace(',','.'));
            return rs;
        }
    }
}
