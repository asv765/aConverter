create procedure CNV$CNV_00900_LCHARS (
    NEEDDELETE smallint)
as
...
BEGIN
    ...
    DO BEGIN
        INSERT INTO documents (documentcd, registerusercd, otvetstvusercd, docname, factdocumentdate)
            VALUES (GEN_ID(documents_gen, 1), 1, 1, 'Импорт качественных характеристик', :date_);
        INSERT INTO lcharsabonentlist (lshet, kodlcharslist, abonentlchardate, documentcd, significance)
            VALUES (:lshet, :lcharcd, :date_, GEN_ID(documents_gen,0), :value_);
    END
END
