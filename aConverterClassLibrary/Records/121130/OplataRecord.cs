...
namespace aConverterClassLibrary.Records
{
    ...
    [Index("COMLEX", "LSHET + STR(SERVICECD,5) + STR(YEAR,4) + STR(MONTH,2)")]
    public class OplataRecord: AbstractRecord
    {
        ...
        // SUMMA N(13,2)
        // </summary>
        [FieldName("SUMMA"), FieldType('N'), FieldWidth(13), FieldDec(2), FieldDescription("Сумма оплаты")]
        ...
        {
            string rs = String.Format("INSERT INTO oplata (LSHET, MONTH, YEAR, SUMMA, DATE, DATE_VV, SOURCECD, SOURCENAME, SERVICECD, SERVICENAM, PRIM_) VALUES ('{0}', {1}, {2}, {3}, CTOD('{4}'), CTOD('{5}'), {6}, '{7}', {8}, '{9}', '{10}')", String.IsNullOrEmpty(Lshet) ? "" : Lshet.Trim(), Month.ToString(), Year.ToString(), Summa.ToString().Replace(',', '.'), Date == DateTime.MinValue ? "" : String.Format("{0}/{1}/{2}", Date.Month, Date.Day, Date.Year), Date_vv == DateTime.MinValue ? "" : String.Format("{0}/{1}/{2}", Date_vv.Month, Date_vv.Day, Date_vv.Year), Sourcecd.ToString(), String.IsNullOrEmpty(Sourcename) ? "" : Sourcename.Trim(), Servicecd.ToString(), String.IsNullOrEmpty(Servicenam) ? "" : Servicenam.Trim(), String.IsNullOrEmpty(Prim_) ? "" : Prim_.Trim());
            return rs;
        }
    }
}
