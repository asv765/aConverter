...
declare variable ADDCHARID INTEGER;
declare variable ADDCHARVALUE VARCHAR(2000);
declare variable contractid integer;
begin
    FOR SELECT CA.LSHET, CA.NAME, CA.TYPEID, CA.DOC_SER, CA.DOC_NUMBER, CA.DOC_DATE, CA.ORGID, CA.STARTDATE, CA.ENDDATE, CA.PRIM, CA.CITIZENID, CA.SERVICES, CA.ID
        FROM CNV$ABONENTCONTRACT CA
    INTO :LSHET, :NAME, :TYPEID, :DOC_SER, :DOC_NUMBER, :DOC_DATE, :ORGID, :STARTDATE, :ENDDATE, :PRIM, :CITIZENID, :SERVICES, :CONTRACTID
    DO BEGIN
		...
		BEGIN
			...
			END
		END
        FOR SELECT ADDCHARCD, VALUE_
            FROM CNV$CONTRACTADDCHAR
			WHERE CONTRACTID = :CONTRACTID
        INTO :ADDCHARID, :ADDCHARVALUE
        DO BEGIN
            INSERT INTO DOCUMENTADDITIONALCHARS (DOCUMENTCD, ADDITIONALCHARCD, SIGNIFICANCE) VALUES (:DOCUMENTCD, :ADDCHARID, :ADDCHARVALUE);
        END
    END
end^
