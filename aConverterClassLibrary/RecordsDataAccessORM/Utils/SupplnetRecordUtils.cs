...
namespace aConverterClassLibrary.RecordsDataAccessORM
{
    public class SupplnetRecordUtils
    {
        ...
        public static List<CNV_SUPPLNET> Normalize(List<CNV_SUPPLNET> crl)
        {
            // Сортируем список
            List<CNV_SUPPLNET> rcrl = new List<CNV_SUPPLNET>();
            crl.Sort(CompareSupplnet);
            rcrl.AddRange(crl);
            // Удаляем дублирующиеся строки
            CNV_SUPPLNET oldsr = crl[0];
            for (int i = 0; i < crl.Count; i++)
            {
                ...
                {
                    CNV_SUPPLNET newsr = oldsr.CloneRecord();
        ...
        /// <returns></returns>
        public static int CompareSupplnet(CNV_SUPPLNET sr1, CNV_SUPPLNET sr2)
        {
