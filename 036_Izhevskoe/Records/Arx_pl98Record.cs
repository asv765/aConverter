...
{
    [TableName("ARX_PL98.DBF")]
    public partial class Arx_pl98Record: AbstractRecord
    {
        ...
        {
            Arx_pl98Record retValue = new Arx_pl98Record();
        ...
        {
            string rs = String.Format("INSERT INTO ARX_PL98 (N_LITS, MES_PL, S_KVART, S_NAIM, S_WATER, S_KANAL, S_MUSOR, GOD, KOROV) VALUES ({0}, '{1}', {2}, {3}, {4}, {5}, {6}, {7}, {8})", N_lits.ToString(), String.IsNullOrEmpty(Mes_pl) ? "" : Mes_pl.Trim(), S_kvart.ToString().Replace(',','.'), S_naim.ToString().Replace(',','.'), S_water.ToString().Replace(',','.'), S_kanal.ToString().Replace(',','.'), S_musor.ToString().Replace(',','.'), God.ToString(), Korov.ToString().Replace(',','.'));
            return rs;
        }
    }
}
